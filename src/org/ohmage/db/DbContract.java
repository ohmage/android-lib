...
/**
 * Contract class for interacting with {@link DbProvider}. Defines the kinds of
 * entities managed by the provider, their schemas, and their relationships.
 * 
 * @author faisal
 */
public class DbContract {
    ...
    interface CampaignColumns {
        ...
        /**
         * Update time in db so we don't change things which we updated more
         * recently
         */
    ...
    interface ResponseColumns {
        ...
        /**
         * LOCATION_-prefixed final string from {@link SurveyGeotagService}; if
         * LOCATION_UNAVAILABLE is chosen, location data is ignored
         */
        ...
        /**
         * the context in which the survey was launched (e.g. triggered,
         * user-initiated, etc.)
         */
        ...
        /**
         * read-only, an int indicating the status of a response; use constants
         * supplied in this class (e.g. STATUS_UPLOADED)
         */
    ...
    public static final class Campaigns implements BaseColumns, CampaignColumns {
        ...
        public static final Uri CONTENT_URI = BASE_CONTENT_URI.buildUpon()
                .appendPath(PATH_CAMPAIGNS).build();
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.ohmage.campaign";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.ohmage.campaign";
        ...
        /**
         * Build {@link Uri} that references any {@link Surveys} associated with
         * the requested {@link #CAMPAIGN_URN}.
         */
        ...
        /**
         * Build {@link Uri} that references any {@link Surveys} associated with
         * the requested {@link #CAMPAIGN_URN} and {@link Surveys#SURVEY_ID}
         */
        ...
        /**
         * Build {@link Uri} that references any {@link SurveyPrompts}
         * associated with the requested {@link #CAMPAIGN_URN} and
         * {@link Surveys#SURVEY_ID}
         */
        public static Uri buildSurveyPromptsUri(String campaignUrn, String surveyId) {
            return buildSurveysUri(campaignUrn, surveyId).buildUpon().appendPath(PATH_SURVEYS)
                    .build();
        }
        ...
        public static Uri buildResponsesUri(String campaignUrn) {
            return CONTENT_URI.buildUpon().appendPath(campaignUrn).appendPath(PATH_RESPONSES)
                    .build();
        }

        /**
         * Build {@link Uri} that references any {@link Responses} associated
         * with the requested {@link #CAMPAIGN_URN} and
         * {@link Surveys#SURVEY_ID}
         */
        public static Uri buildResponsesUri(String campaignUrn, String surveyId) {
            return buildSurveysUri(campaignUrn, surveyId).buildUpon().appendPath(PATH_RESPONSES)
                    .build();
        }
    ...
    public static final class Surveys implements BaseColumns, SurveyColumns {
        ...
        public static final Uri CONTENT_URI = BASE_CONTENT_URI.buildUpon().appendPath(PATH_SURVEYS)
                .build();
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.ohmage.survey";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.ohmage.survey";
    ...
    /**
     * Represents a prompt within a survey, again as extracted from the campaign
     * XML.
     */
    public static final class SurveyPrompts implements BaseColumns, SurveyPromptColumns {
        ...
        public static final Uri CONTENT_URI = BASE_CONTENT_URI.buildUpon().appendPath(PATH_SURVEYS)
                .build();
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.ohmage.surveyprompt";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.ohmage.surveyprompt";
    ...
    /**
     * Represents a survey response. The 'source' field indicates how the survey
     * response was added to the database. If it's "local", that means that the
     * survey was filled out by the user and is awaiting upload. If it's
     * "remote", it means that it was downloaded by the feedback sync service.
     * Both "local" and "remote" surveys are used for generating feedback
     * visualizations. The 'uploaded' field indicates whether or not the survey
     * has been uploaded by the {@link OldUploadService} yet. This field is
     * valid only for records where 'source' is "local".
     */
    public static final class Responses implements BaseColumns, ResponseColumns {
        ...
        public static boolean isResponseUri(Uri uri) {
            ...
            return segments != null && segments.size() > 0
                    && PATH_RESPONSES.equals(segments.get(0));
        }
    }
    ...
    /**
     * Represents a single response to a prompt. These are extracted from the
     * survey response json at the time of survey completion, or in
     * ResponseSyncService from the downloaded response data.
     */
    public static final class PromptResponses implements BaseColumns, PromptResponseColumns {

        public static final Uri CONTENT_URI = BASE_CONTENT_URI.buildUpon().appendPath(PATH_PROMPTS)
                .build();
        public static final String CONTENT_TYPE = "vnd.android.cursor.dir/vnd.ohmage.promptresponse";
        public static final String CONTENT_ITEM_TYPE = "vnd.android.cursor.item/vnd.ohmage.promptresponse";

        public enum AggregateTypes {
            AVG, COUNT, MAX, MIN, TOTAL
        }
        ...
        /**
         * Read Prompt id from {@link PromptResponses} {@link Uri}. which has a
         * survey
         */
        public static String getSurveyPromptId(Uri uri) {
        ...
        public static Uri getPromptsByCampaignAndSurvey(String campaignUrn, String surveyID,
                String promptID) {
            return BASE_CONTENT_URI.buildUpon().appendPath(PATH_CAMPAIGNS).appendPath(campaignUrn)
                    .appendPath(PATH_SURVEYS).appendPath(surveyID).appendPath(PATH_RESPONSES)
                    .appendPath(PATH_PROMPTS).appendPath(promptID).build();
        }

        /**
         * Returns the prompts for a specific promptID This uri is only useful
         * in certain circumstances where the same prompt is used for more than
         * one survey like it is in NIH
         * 
         * @param campaignUrn
         * @param promptID
         * @return
        ...
        public static Uri getPromptsByCampaign(String campaignUrn, String promptID) {
            return BASE_CONTENT_URI.buildUpon().appendPath(PATH_CAMPAIGNS).appendPath(campaignUrn)
                    .appendPath(PATH_RESPONSES).appendPath(PATH_PROMPTS).appendPath(promptID)
                    .build();
        }
    }
}
